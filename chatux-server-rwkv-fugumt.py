...
if not PILE_v2_MODEL:
    RWKVpipeline = PIPELINE(model, f"{current_path}/20B_tokenizer.json")
    END_OF_TEXT = 0
    END_OF_LINE = 187
else:
    RWKVpipeline = PIPELINE(model, "cl100k_base")
    END_OF_TEXT = 100257
    END_OF_LINE = 198
...
out = run_rnn(RWKVpipeline.encode(init_prompt))
...
def handle_message(message):
    ...
    if msg == '+reset':
        ...
        if msg[:5].lower() == '+gen ':
            ...
            model_state = None
            model_tokens = []
            out = run_rnn(RWKVpipeline.encode(new))
...
'''
        ...
        for i in range(FREE_GEN_LEN + 100):
            ...
            token = RWKVpipeline.sample_logits(
                out,
                temperature=x_temp,
                top_p=x_top_p,
            )
            ...
            xxx = RWKVpipeline.decode(model_tokens[out_last:])
            if '\ufffd' not in xxx:  # avoid utf-8 display issues
                ...
                if i >= FREE_GEN_LEN:
                    break
        print('\n')
        send_msg = RWKVpipeline.decode(model_tokens[begin:]).strip()
        # print(f'### send ###\n[{send_msg}]')
        ...
        if msg.lower() == '+':
            ...
            # print(f'### add ###\n[{new}]')
            out = run_rnn(RWKVpipeline.encode(new), newline_adj=-999999999)
        ...
        for i in range(999):
            ...
            send_msg = RWKVpipeline.decode(model_tokens[begin:])
            if '\n\n' in send_msg:
...
@app.get("/chat_api_org")
async def chat_org(text: str = ""):
...
async def chat(text: str = ""):
    ...
    if is_halfwidth(input_text) == True:
        input_text_trans = input_text
        input_text_trans_str = input_text_trans
    else:
        input_text_trans = je_translator(input_text)
        input_text_trans_str = dict(input_text_trans[0])['translation_text']

    reply = handle_message(input_text_trans_str).replace('\n', '<br>')
    reply_text_trans = ej_translator(reply)
    reply_text_trans_str = str(reply_text_trans).replace("[{'translation_text': '", "").replace("'}]", "") + "\n"
    
    print(f'input:{text} ({input_text_trans_str})' + '\n')
    print(f'reply:{reply}' + '\n')
    print(f'翻訳結果:{reply_text_trans_str}' + '\n')

    outJson = {
        "output": [
            {
                "type": "text",
                "value": reply + "<br>翻訳結果: " + reply_text_trans_str
            }
        ]
    }
    return outJson;
